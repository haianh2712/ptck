    ...
    def parse(self, file_path):
        ...
        try:
            ...
            if sh_tien:
                ...
                # [QUAN TRỌNG] ĐẢO NGƯỢC DATAFRAME (CŨ -> MỚI)
                # Để đảm bảo Snapshot cuối ngày được xử lý sau cùng
                df_t = df_t.iloc[::-1]
                    ...
                    for _, row in df_t.iterrows():
                        try:
                            ...
                            if c_bal:
                                ...
                                events.append({'date': d_obj, 'type': 'CASH_SNAPSHOT', 'val': val_bal, 'source': 'VPS_LEDGER'})
                            ...
                            # XỬ LÝ NẠP & CỔ TỨC
                            if val_in > 0:
                                if any(k in desc.lower() for k in ['cashin', 'nộp tiền', 'nop tien', 'chuyen tien vao']):
                                    if 'nhận tiền bán' not in desc.lower() and 'hoàn' not in desc.lower():
                                        events.append({'date': d_obj, 'type': 'DEPOSIT', 'val': val_in, 'source': 'VPS'})

                                if any(k in desc.lower() for k in ['cổ tức', 'co tuc', 'div', 'lãi', 'quyền']):
            ...
            if sh_cp:
                ...
                # Cũng đảo ngược CP cho đúng trình tự thời gian
                df_cp = df_cp.iloc[::-1]
                    ...
                    for _, row in df_cp.iterrows():
                        try:
                            ...
                            if val_in > 0:
                                ...
                                if not is_zero:
                                    # Tăng phạm vi tìm kiếm giá lên +/- 10 ngày để bắt các lệnh lệch ngày
                                    for delta in range(-10, 11):
                                        chk_d = d_obj + timedelta(days=delta)
                                        k = (sym, chk_d.strftime('%Y-%m-%d'))
                                        if k in price_map:
                                            price = price_map[k]; break
        ...
        # Ưu tiên Snapshot thấp nhất (99) để nó được xử lý cuối cùng trong ngày (chốt sổ)
        type_prio = {'DEPOSIT': 1, 'BUY': 2, 'SELL': 3, 'PNL_UPDATE': 4, 'DIVIDEND': 5, 'CASH_SNAPSHOT': 99}
